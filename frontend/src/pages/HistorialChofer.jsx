// src/pages/HistorialChofer.jsx

import React, { useState, useEffect, useMemo } from 'react';
import { useNavigate } from 'react-router-dom';
import apiClient from '../api/axios.js';
import styles from '../css/gestionagenda.module.css'; // Reutilizamos un estilo de tabla que ya tienes
...
export default function HistorialChofer() {
    ...
    useEffect(() => {
        const fetchHistorial = async () => {
            try {
                // 1. Hacemos la llamada al endpoint que ya nos filtra por chofer
                const response = await apiClient.get('/ordenes/');
                const todasLasOrdenes = response.data.results || response.data || [];
                
                // 2. Filtramos en el frontend para quedarnos solo con las finalizadas
                const finalizadas = todasLasOrdenes.filter(o => o.estado === 'Finalizado');

                // 3. Ordenamos por fecha de entrega, de la más reciente a la más antigua
