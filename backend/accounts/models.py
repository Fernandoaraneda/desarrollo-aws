...
class Taller(models.Model):
    nombre = models.CharField(
        max_length=100,
        unique=True,
        help_text="Nombre descriptivo del taller (Ej: Pepsico Maipú)",
    )
    direccion = models.CharField(
        max_length=255, help_text="Dirección completa del taller"
    )
...
class Agendamiento(TimeStampedModel):
    ...
    solicita_grua = models.BooleanField(
        default=False,
        help_text="Marcar si el vehículo requiere ser movilizado por una grúa.",
    )
    direccion_grua = models.CharField(
        max_length=255,
        null=True,
        blank=True,
        help_text="Dirección de retiro para la grúa",
    )
    grua_enviada = models.BooleanField(
        default=False, help_text="Marca si la grúa ya fue despachada"
    )
    vehiculo = models.ForeignKey(
        Vehiculo, on_delete=models.PROTECT, related_name="agendamientos"
    )
    chofer_asociado = models.ForeignKey(
        Usuario,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
    ...
    estado = models.CharField(
        max_length=50, choices=Estado.choices, default=Estado.PROGRAMADO, db_index=True
    )
    ...
    creado_por = models.ForeignKey(
        Usuario, on_delete=models.PROTECT, related_name="agendamientos_creados"
    )
    ...
    def save(self, *args, **kwargs):
        if self.fecha_hora_programada and self.duracion_estimada_minutos:
            self.fecha_hora_fin = self.fecha_hora_programada + timedelta(
                minutes=self.duracion_estimada_minutos
            )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                ...
                condition=~Q(estado__in=["Finalizado", "Cancelado"]),
...
class AgendamientoHistorial(models.Model):
    agendamiento = models.ForeignKey(
        Agendamiento, on_delete=models.CASCADE, related_name="historial"
    )
...
class AgendamientoDocumento(models.Model):
    agendamiento = models.ForeignKey(
        Agendamiento, on_delete=models.CASCADE, related_name="documentos"
    )
    tipo = models.CharField(
        max_length=50,
        choices=[("Foto", "Foto"), ("Informe", "Informe"), ("Otro", "Otro")],
    )
...
class Orden(TimeStampedModel):
    ...
    agendamiento_origen = models.OneToOneField(
        Agendamiento,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
    ...
    estado = models.CharField(
        max_length=50, choices=Estado.choices, default=Estado.INGRESADO, db_index=True
    )
    descripcion_falla = models.TextField("Descripción de la Falla (Cliente)")
    diagnostico_tecnico = models.TextField(
        "Diagnóstico Técnico (Mecánico)", blank=True, null=True
    )
    ...
    def __str__(self):
        return (
            f"Orden #{self.id} - {self.vehiculo.patente} ({self.get_estado_display()})"
        )
...
class OrdenDocumento(models.Model):
    ...
    orden = models.ForeignKey(
        Orden, on_delete=models.CASCADE, related_name="documentos"
    )
    ...
    fecha = models.DateTimeField(auto_now_add=True)
    subido_por = models.ForeignKey(Usuario, on_delete=models.PROTECT)
    estado_en_carga = models.CharField(
        "Estado al Cargar", max_length=50, blank=True, null=True, db_index=True
    )
...
class OrdenItem(models.Model):
    ...
    motivo_gestion = models.TextField(
        blank=True, null=True
    )  # Para el motivo de rechazo
    ...
    servicio = models.ForeignKey(
        Servicio, on_delete=models.SET_NULL, blank=True, null=True
    )
    ...
    def clean(self):
        if (self.producto and self.servicio) or (
            not self.producto and not self.servicio
        ):
            raise ValidationError(
                "Debe especificar un producto o un servicio, pero no ambos."
            )
...
class LlaveVehiculo(TimeStampedModel):
    ...
    vehiculo = models.ForeignKey(
        Vehiculo, on_delete=models.PROTECT, related_name="llaves"
    )

    codigo_interno = models.CharField(
        max_length=50, unique=True, help_text="Código único o ID del llavero físico."
    )
    tipo = models.CharField(max_length=50, choices=Tipo.choices, default=Tipo.ORIGINAL)
    estado = models.CharField(
        max_length=50, choices=Estado.choices, default=Estado.EN_BODEGA, db_index=True
    )
...
class PrestamoLlave(TimeStampedModel):
    ...
    llave = models.ForeignKey(
        LlaveVehiculo, on_delete=models.PROTECT, related_name="prestamos"
    )
    usuario_retira = models.ForeignKey(
        Usuario, on_delete=models.PROTECT, related_name="prestamos_realizados"
    )
...
class LlaveHistorialEstado(TimeStampedModel):
    ...
    llave = models.ForeignKey(
        LlaveVehiculo, on_delete=models.CASCADE, related_name="historial_estados"
    )
    usuario_reporta = models.ForeignKey(
        Usuario, on_delete=models.PROTECT, related_name="reportes_llave_realizados"
    )
...
class ChatMessage(TimeStampedModel):
    ...
    def __str__(self):
        autor_nombre = self.autor.username if self.autor else "Usuario Eliminado"
        return f"Mensaje de {autor_nombre}"
